<div class="mt-8">
    <div class="mt-8">
        ...
        <div class="mb-8 rounded-2 d-flex flex-column gap-6" class="w-100">
            ...
            @if ( !$editComment )
                <div class="d-flex gap-2" wire:loading.class="d-none" wire:target="storeComment">
                    <div class="rounded-2 w-100">
                        <div class="d-flex align-items-cen  ter gap-1">
                            <p class="fw-bold mb-1"> {{ auth()->user()->name }} </p>
                        </div>
                        <div>
                            <textarea wire:model.defer="comment" id="comment" class="form-control mb-2 rounded-3" placeholder="Tambahkan komentar" rows="3" onkeyup="setDisabledButtonComment()">{{ $comment }}</textarea>
            ...
            @foreach ( $news->comments as $item)
                ...
                @if ( $item->id == $commentId && $editComment)
                    <div class="d-flex gap-2" wire:loading.class="d-none" wire:target="updateComment">
                        <div class="rounded-2 w-100">
                            <div class="d-flex align-items-cen  ter gap-1">
                                <p class="fw-bold mb-1"> {{ auth()->user()->name }} </p>
                            </div>
                            <div>
                                <textarea wire:model.defer="comment" id="edit-comment" class="form-control mb-2 rounded-3" placeholder="Tambahkan komentar" rows="3" onkeyup="setDisabledButtonComment()">{{ $comment }}</textarea>
                                <div class="text-end w-100">
                                    <button class="btn btn-light btn-sm" wire:click="cancelEditComment"> Batal </button>
                                    <button class="btn btn-primary btn-sm" id="btn-update-komentar" wire:click="updateComment('{{ $commentId  }}')"> Komentar </button>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <span class="text-muted fs-8">
                                        -
                                        @if ($item->updated_at)
                                            {{ $item->updated_at->locale('id_ID')->diffForHumans() }} (diedit)
                                        @else
                                            {{ $item->created_at->locale('id_ID')->diffForHumans() }}
                                        @endif
                                    </span>
                                </div>
                                @if ( auth()->user()->id == $item->user_id )
                                    <span  data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" class="cursor-pointer">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-150px py-4" data-kt-menu="true" style="">
                                        <div class="menu-item px-3">
                                            <a href="javascript:;" wire:click="setComment('{{ $item->id }}')" class="menu-link px-3">
